...
io.on('connection', function (socket) {
    ...
    socket.on('reference-listening', function (data) {
         ...
         */
            ...
            var updatedObject = data.objects;
            updatedObject['_id'] = objectId(data._id);

            db.collection(data.reference).update({_id: objectId(data._id)}, data.objects, {multi: false}, function (err, docsUpdate) {

                if (!err) {

                    socket.emit(data.reference, { operation: data.operation, objects: {response: docsUpdate, data: updatedObject} });
                    socket.broadcast.emit(data.reference, { operation: data.operation, objects: {response: docsUpdate, data: updatedObject} });
         ...
         */
            ...
            db.collection(data.reference).findOne({_id: objectId(data._id)}, function (err, docsGet) {

                if (!err) {

                    db.collection(data.reference).remove({_id: objectId(data._id)}, {safe: true}, function (err, docsDelete) {

                        if (!err) {
                            socket.emit(data.reference, { operation: data.operation, objects: {response: docsDelete, data: docsGet} });
                            socket.broadcast.emit(data.reference, { operation: data.operation, objects: {response: docsDelete, data: docsGet} });
                        }
